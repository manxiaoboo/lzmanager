...
const paths = {
    ...
    server: {
        ...
        //   `${serverPath}/api/spyderhub/seeds/*`, 
        //   `${serverPath}/api/spyderhub/backup/*`
        ],
        json: [`${serverPath}/**/*.json`], //`!${serverPath}/api/spyderhub/seeds/*.json`, `!${serverPath}/api/spyderhub/backup/*.json`],
        test: {
...
gulp.task('start:server:prod', () => {
    ...
    nodemon(`-w ${paths.dist}/${serverPath} ${paths.dist}/${serverPath}  --ignore ${paths.dist}/${serverPath}/api/spyderhub/backup/*`)
        .on('log', onServerLog);
});
...
gulp.task('start:server', () => {
    ...
    nodemon(`-w ${serverPath} ${serverPath} --ignore ${serverPath}/api/spyderhub/backup/*`)
        .on('log', onServerLog);
});
...
gulp.task('copy:fonts', () => {
    return gulp.src(`${clientPath}/bower_components/{bootstrap,fontawesome}/fonts/**/*`, { dot: true })
...
// add seeds copy - by zb 2016-4-21
